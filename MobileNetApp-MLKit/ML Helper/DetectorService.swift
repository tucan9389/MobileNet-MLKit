...
public enum DetectorError: Int, CustomNSError {
    ...
    public static var errorDomain: String { return "com.tucan9389.mobilenetapp-mlkit.detectorservice" }
...
public class DetectorService: NSObject {
    ...
    public func loadModel(options: ModelOptions,
                          labelsPath: String,
                          kr_labelsPath: String?,
        ...
        do {
            labels = try loadLabels(from: labelsPath, from: kr_labelsPath, with: DetectorConstants.labelsSeparator)
    ...
    func loadLabels(from labelsPath: String,
                    from kr_labelsPath: String?,
                    with labelsSeparator: String) throws -> [String] {
        
        let encoding = String.Encoding.utf8.rawValue
        let contents = try NSString(contentsOfFile: labelsPath, encoding: encoding)
        var labels = contents.components(separatedBy: labelsSeparator)
        if let lastLabel = labels.last, lastLabel.count == 0 {
            labels.removeLast()
        }
        
        if let kr_labelsPath = kr_labelsPath {
            let contents = try NSString(contentsOfFile: kr_labelsPath, encoding: encoding)
            var kr_labels = contents.components(separatedBy: labelsSeparator)
            if let lastLabel = kr_labels.last, lastLabel.count == 0 {
                kr_labels.removeLast()
            }
            var translatedLabels = labels
            for (i, (enLabel, krLabel)) in zip(labels, kr_labels).enumerated() {
                translatedLabels[i] = "\(enLabel)(\(krLabel))"
            }
